...
fn deploy_factory() -> (ContractAddress, ContractAddress, IStrapexDispatcher, ContractAddress) {
    ...
    let mock_token_class = declare("Mock").unwrap().contract_class();
    let owner: ContractAddress = contract_address_const::<OWNER>();
    let constructor_calldata: Array<felt252> = array![
        TOKEN, SYMBOL, INITIAL_SUPPLY.try_into().unwrap(), owner.into(),
    ];
    let (token_address, _) = mock_token_class.deploy(@constructor_calldata).unwrap();
    ...
    let constructor_data: Array<felt252> = array![
        owner.into(), child_hash_felt, token_address.into(),
    ];
    ...
    (child_addr, owner, child_disp, token_address)
}
